...
TEST_CASE("SampleSet Test #1: length", "[samples][sample_set]")
{
    ...
    REQUIRE(getSamplesForPreset("default-preset-1").length() == 3);
    REQUIRE(getSamplesForPreset("default-preset-2").length() == 8);
    REQUIRE(getSamplesForPreset("default-preset-3").length() == 8);
}
...
TEST_CASE("SampleSet Test #3: get", "[samples][sample_set]")
{
    ...
    REQUIRE(samples.get(42).filepath.string() == "/usr/local/include/MySampler/samples/Pad/instrument-oneshots-by-fredricch.pads.pad ~ tropical.wav");
    REQUIRE(samples.get(42).rootNote == 60);

    REQUIRE(samples.get(54).filepath.string() == "/usr/local/include/MySampler/samples/Synth/oneshots-from-songs-by-allstar2x.Juice Wrld - Armed & Dangerous Lead.wav");
    REQUIRE(samples.get(54).rootNote == 48);
}
