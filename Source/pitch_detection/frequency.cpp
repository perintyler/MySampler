/*** Piano960 | pitch_detection/frequency.cpp ***/
...
Note getNoteForFrequency(float frequency)
{
    ...
    int noteID;
    for (noteID = 0; noteID < NOTE_FREQUENCIES.size()-1; ++noteID) {
        
        float frequencyOfLowerNote = NOTE_FREQUENCIES[noteID];
        float frequencyOfHigherNote = NOTE_FREQUENCIES[noteID+1];
...
{
    int semitoneNumber = getNoteForFrequency(frequency) % OCTAVE_SIZE;
